...
#[inline(always)]
#[must_use]
pub fn evaluate_quadratic_fixed(a: i32, b: i32, c: i32, x: i32) -> i32 {
    let x64 = x as i64;
...
#[inline(always)]
#[must_use]
pub fn evaluate_cubic_fixed(a: i32, b: i32, c: i32, d: i32, x: i32) -> i32 {
    let x64 = x as i64;
    let x2 = x64 * x64;
    let x3 = x2 * x64;
...
mod tests {
    ...
    fn test_negative_slope() {
        ...
        // slope should be negative: -50 in Q16.16 = -3276800
        assert!(slope < 0);
        assert!((slope + 3276800).abs() < 500);
    }
    ...
    fn test_fit_quadratic_perfect() {
        ...
        // a ≈ 1.0 (Q16.16 = 65536)
        assert!(
            (a - int_to_q16(1)).abs() < 500,
            "a = {} (expected ~65536)",
            a
        );
        // b ≈ 0
        assert!(b.abs() < 500, "b = {} (expected ~0)", b);
        // c ≈ 0
        assert!(c.abs() < 500, "c = {} (expected ~0)", c);
    }
    ...
    fn test_fit_cubic_perfect() {
        ...
        // b ≈ 0 (x² 係数)
        assert!(b.abs() < 2000, "b = {} (expected ~0)", b);
    }
    ...
    fn test_delta_encoding_savings() {
        ...
        assert!(delta <= original, "delta={} original={}", delta, original);
    }
